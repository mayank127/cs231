...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, gamma, beta, bn_param, u, std)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    x, gamma, beta, bn_param,u,std = cache
    eps = bn_param.get('eps', 1e-5)
    ...
    dgamma = np.sum((nume * dout)/denom, axis=0)
    
    dStep1 = gamma * dout
    
    dStep2 = (dStep1/denom)
    
    dStep3 = np.sum(nume*dStep1, axis=0)
    dStep4 = (-1.0 / (denom**2)) * dStep3
    dStep5 = dStep4 / (2.0 * denom * N)
    
    dStep6 = 2.0 * nume * dStep5 + dStep2
    
    dStep7 = np.sum(dStep6, axis=0) * -1.0/N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum((x-u) * dout/std, axis=0)
    N = float(x.shape[0])
    dy = gamma * dout
        
    dx = 1/std * dy - 1/(N*std) * np.sum(dy, axis=0) - (x-u)/(N*std**3) * np.sum((x-u)*dy, axis=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    out = gamma * ((xT - u) / std).T + beta
    cache = (x, gamma, beta, ln_param, u, std)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def layernorm_backward(dout, cache):
    ...
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(((xT-u)/std).T * dout, axis=0)
    N = float(xT.shape[0])
    dy = (gamma * dout).T
        
    dx = (1/std * dy - 1/(N*std) * np.sum(dy, axis=0) - (xT-u)/(N*std**3) * np.sum((xT-u)*dy, axis=0)).T
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dgamma, dbeta
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(0, H+2*pad-HH+1, stride):
        ...
        for j in range(0, W+2*pad-WW+1, stride):
            out[:,:,ida,jda] = np.sum(xpad[:,None,:,i:i+HH,j:j+WW]*w[None,:,:,:,:], axis=(2,3,4)) + b
            jda += 1
        ida += 1
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(0, H+2*pad-HH+1, stride):
        ...
        for j in range(0, W+2*pad-WW+1, stride):
            dij = dout[:,:,ida,jda]
            dxpad[:,:,i:i+HH,j:j+WW] += np.sum(dij[:,:,None,None,None]*w[None,:,:,:,:], axis=1)
            dw += np.sum(dij[:,:,None,None,None] * xpad[:,None,:,i:i+HH,j:j+WW], axis=0)
            db += np.sum(dij, axis=0)
            jda += 1
        ida += 1
...
def max_pool_forward_naive(x, pool_param):
    ...
    Hd = 1 + (H - pool_height) // stride
    Wd = 1 + (W - pool_width) // stride
    
    out = np.zeros((N,C,Hd,Wd))
    ida = 0
    jda = 0
    for i in range(0, H-pool_height//2+1, stride):
        jda = 0
        for j in range(0, W-pool_width//2+1, stride):
            out[:,:,ida,jda] = np.max(x[:,:,i:i+pool_height,j:j+pool_width], axis=(2,3))
            jda += 1
        ida += 1
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(0, H-pool_height//2+1, stride):
        ...
        for j in range(0, W-pool_width//2+1, stride):
            for n in range(N):
                for c in range(C):
                    subx = x[n,c,i:i+pool_height,j:j+pool_width]
                    index = np.unravel_index(np.argmax(subx),subx.shape) 
                    dx[n,c,i+index[0],j+index[1]] += dout[n,c,ida,jda]
            jda += 1
        ida += 1
    ###########################################################################
    #                             END OF YOUR CODE                            #
